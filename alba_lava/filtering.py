...
#TODO: Create MTTime and MTCorr combined variable - but also add additional filtering rules for timed tasks to account for incomplete tasks (i.e. didn't reach ceiling but time is less than 120?)
...
# # print how many unique values and ocunts are in df_lava['neuropsychbedside']['LngPVrb']
# df = df_lava['neuropsychbedside']
# df['PPVT'] = df.apply(lambda x: x['LngPVrb'] + x['LngPDes'] + x['LngPAni'] + x['LngPIna'] if pd.notnull(x['LngPVrb']) and pd.notnull(x['LngPDes']) and pd.notnull(x['LngPAni']) and pd.notnull(x['LngPIna']) else np.nan, axis=1)
# df['PPVT'].value_counts()

# df['MTTime'].unique()

# df['MTCorr'].value_counts()
...
def filter_other_cog(df):
    ...
    df = df.groupby('HELPERID',group_keys=True).apply(lambda x: x.sort_values('DCDate', ascending=False).ffill(limit=1).bfill(limit=1))
    return (df)
###############################################################################
# Filter Timepoints:
def filter_timepoints(df, timepoint='first'):
    ...
    'all' = All DCDate timepoints for each PIDN
    """
